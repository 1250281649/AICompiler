...
TVM_FFI_STATIC_INIT_BLOCK({
  ...
  refl::GlobalDef()
      .def_packed(
          "tvm.contrib.miopen.conv2d.setup",
          [](ffi::PackedArgs args, ffi::Any* ret) {
            ...
            entry_ptr->conv_entry.data_type = static_cast<miopenDataType_t>(
                dtype);  // MIOpen supports fp32(miopenFloat),
                         // fp16(miopenHalf), int32, int8 at this moment.
            // Set Desc
            MIOPEN_CALL(miopenInitConvolutionDescriptor(
                entry_ptr->conv_entry.conv_desc, entry_ptr->conv_entry.mode, pad_h, pad_w, stride_h,
            ...
            MIOPEN_CALL(miopenSet4dTensorDescriptor(entry_ptr->conv_entry.filter_desc,
                                                    entry_ptr->conv_entry.data_type, w_dim0,
                                                    w_dim1 / n_group, w_dim2, w_dim3));
            // Set Input
            ...
            MIOPEN_CALL(miopenSet4dTensorDescriptor(entry_ptr->conv_entry.output_desc,
                                                    entry_ptr->conv_entry.data_type, oshape[0],
                                                    oshape[1], oshape[2], oshape[3]));
            ...
            float* filter_buf = static_cast<float*>(rocm_api->AllocWorkspace(
                entry_ptr->conv_entry.device, filter_size * sizeof(float)));
            float* output_buf = static_cast<float*>(rocm_api->AllocWorkspace(
                entry_ptr->conv_entry.device, output_size * sizeof(float)));
            ...
            MIOPEN_CALL(miopenFindConvolutionForwardAlgorithm(
                ...
                entry_ptr->conv_entry.output_desc, output_buf, request_algo_count,
                &returned_algo_count, perfs, workspace, workspace_size, exhaustive_search));
